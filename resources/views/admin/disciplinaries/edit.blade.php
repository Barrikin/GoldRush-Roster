...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.disciplinary.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.disciplinaries.update", [$disciplinary->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('officer') ? 'is-invalid' : '' }}" name="officer_id" id="officer_id" required>
                    @foreach($officers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('officer_id') ? old('officer_id') : $disciplinary->officer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $disciplinary->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('comment') ? 'is-invalid' : '' }}" name="comment" id="comment">{!! old('comment', $disciplinary->comment) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('points') ? 'is-invalid' : '' }}" type="number" name="points" id="points" value="{{ old('points', $disciplinary->points) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('issued_by') ? 'is-invalid' : '' }}" name="issued_by_id" id="issued_by_id" required>
                    @foreach($issued_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('issued_by_id') ? old('issued_by_id') : $disciplinary->issued_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('expire_at') ? 'is-invalid' : '' }}" type="text" name="expire_at" id="expire_at" value="{{ old('expire_at', $disciplinary->expire_at) }}" required>
