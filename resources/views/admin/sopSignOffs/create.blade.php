...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.sopSignOff.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sop-sign-offs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sop') ? 'is-invalid' : '' }}" name="sop_id" id="sop_id" required>
                    @foreach($sops as $id => $entry)
                        <option value="{{ $id }}" {{ old('sop_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sop'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('signed_off_at') ? 'is-invalid' : '' }}" type="text" name="signed_off_at" id="signed_off_at" value="{{ old('signed_off_at') }}">
