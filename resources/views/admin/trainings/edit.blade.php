        ...
        <form method="POST" action="{{ route("admin.trainings.update", [$training->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('officer') ? 'is-invalid' : '' }}" name="officer_id" id="officer_id" required>
                    @foreach($officers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('officer_id') ? old('officer_id') : $training->officer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                    @foreach($courses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $training->course->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('understood') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="understood" id="understood" value="1" {{ $training->understood || old('understood', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('understood_fto') ? 'is-invalid' : '' }}" name="understood_fto_id" id="understood_fto_id">
                    @foreach($understood_ftos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('understood_fto_id') ? old('understood_fto_id') : $training->understood_fto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('understood_at') ? 'is-invalid' : '' }}" type="text" name="understood_at" id="understood_at" value="{{ old('understood_at', $training->understood_at) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('executed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="executed" id="executed" value="1" {{ $training->executed || old('executed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('executed_fto') ? 'is-invalid' : '' }}" name="executed_fto_id" id="executed_fto_id">
                    @foreach($executed_ftos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('executed_fto_id') ? old('executed_fto_id') : $training->executed_fto->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('executed_at') ? 'is-invalid' : '' }}" type="text" name="executed_at" id="executed_at" value="{{ old('executed_at', $training->executed_at) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('comments') ? 'is-invalid' : '' }}" name="comments" id="comments">{!! old('comments', $training->comments) !!}</textarea>
